...
class NewMigrationPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final DesktopRunController runController = ref.watch(desktopRunControllerProvider);
    ...
    return SingleChildScrollView(
      child: Column(
        ...
        children: <Widget>[
          ...
          Text('Step ${state.step} of 3', style: Theme.of(context).textTheme.bodyMedium),
          SizedBox(height: unit.s6),
          if (state.step == 1) NewMigrationSourceTargetStep(state: state, controller: controller),
          if (state.step == 2)
            NewMigrationOptionsStep(state: state, controller: controller, runController: runController),
          if (state.step == 3) NewMigrationPreflightStep(state: state, controller: controller),
        ],
      ),
    );
  }
