...
void main() {
  group('SmokeCommandHandler', () {
    test('forwards RunResult.runWorkdirPath to validator and does not pre-allocate a nested subdir', () async {
      ...
      await runInCurrentDirectory(temp, () async {
        ...
        final Directory smokeRoot = Directory('${temp.path}/smoke-root')..createSync(recursive: true);
        ...
        final _RecordingRunService runService = _RecordingRunService(
          ...
          onRun: (RunRequest request) async {
            ...
            final Directory runDir = Directory('$workdir/run-synthetic')..createSync(recursive: true);
    ...
    test('falls back to provider env aliases when smoke tokens are absent from settings', () async {
      final Directory temp = createTempDir('gfrm-smoke-handler-env-');
      await runInCurrentDirectory(temp, () async {
        final Directory smokeRoot = Directory('${temp.path}/smoke-root')..createSync(recursive: true);
        ...
        final _RecordingRunService runService = _RecordingRunService(
          logger: createSilentLogger(output: BufferConsoleOutput()),
          onRun: (RunRequest request) async {
            final Directory runDir = Directory('${request.options.workdir}/run-synthetic')..createSync(recursive: true);
            File('${runDir.path}/failed-tags.txt').writeAsStringSync('');
            File('${runDir.path}/migration-log.jsonl').writeAsStringSync('');
            File('${runDir.path}/summary.json').writeAsStringSync(
              jsonEncode(<String, dynamic>{
            ...
            return RunResult(
              status: RunStatus.success,
              exitCode: 0,
              resultsRootPath: request.options.workdir,
              runWorkdirPath: runDir.path,
              logPath: '${runDir.path}/migration-log.jsonl',
              checkpointPath: '',
              summaryPath: '${runDir.path}/summary.json',
        ...
        final int exitCode = await handler.run(
          ...
          cwd: temp.path,
          env: const <String, String>{'GH_TOKEN': 'source-alias-token', 'GITLAB_TOKEN': 'target-alias-token'},
          runServiceOverride: runService,
          httpOverride: ScriptedHttpClientHelper(),
        );

        expect(exitCode, 0);
        expect(runService.captured.options.sourceToken, 'source-alias-token');
        expect(runService.captured.options.targetToken, 'target-alias-token');
      });
    });
  });
}
