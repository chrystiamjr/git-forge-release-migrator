  ...
  group('settings flow', () {
    test('setup --yes --local bootstraps env token mappings', () async {
      ...
      await runInCurrentDirectory(temp, () async {
        ...
        final int setupExit = await CliRunner.run(
          <String>[
            commandSetup,
            '--profile',
            'smoke',
    ...
    test('set-token-env and unset-token update local settings file', () async {
      ...
      await runInCurrentDirectory(temp, () async {
        final BufferConsoleOutput setOutput = BufferConsoleOutput();
        final int setExit = await CliRunner.run(
          <String>[
            commandSettings,
        ...
        final BufferConsoleOutput unsetOutput = BufferConsoleOutput();
        final int unsetExit = await CliRunner.run(
          <String>[
            commandSettings,
    ...
    test('settings show masks token_plain values', () async {
      ...
      await runInCurrentDirectory(temp, () async {
        ...
        final BufferConsoleOutput output = BufferConsoleOutput();
        final int exitCode = await CliRunner.run(<String>[commandSettings, settingsActionShow], output: output);
        expect(exitCode, 0);
        expect(output.stdoutLines.single, contains('***'));
        expect(output.stdoutLines.single, isNot(contains('my-secret-token')));
      });
    });
  });
}
