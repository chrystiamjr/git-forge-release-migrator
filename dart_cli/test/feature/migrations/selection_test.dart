...
void main() {
  group('selection', () {
    ...
    test('semverCompare orders semver tags numerically', () {
      expect(SelectionService.semverCompare('v1.2.3', 'v1.2.3'), 0);
      expect(SelectionService.semverCompare('v1.2.3', 'v1.2.4'), lessThan(0));
      expect(SelectionService.semverCompare('v2.0.0', 'v1.9.9'), greaterThan(0));
    });

    test('semverCompare keeps semver before non-semver', () {
      expect(SelectionService.semverCompare('v1.0.0', 'release-1'), lessThan(0));
      expect(SelectionService.semverCompare('release-1', 'v1.0.0'), greaterThan(0));
    });
    ...
    test('collectSelectedTags keeps semver unique and applies inclusive range', () {
      ...
      final List<String> selected = SelectionService.collectSelectedTags(releases, 'v1.0.0', 'v2.0.0');
    ...
    test('releaseByTag returns matching release map', () {
      ...
      final Map<String, dynamic>? hit = SelectionService.releaseByTag(releases, 'v2.0.0');
      final Map<String, dynamic>? miss = SelectionService.releaseByTag(releases, 'v3.0.0');
    ...
    test('loadTagsFile ignores comments and blanks', () {
      ...
      final Set<String> tags = SelectionService.loadTagsFile(tagsPath);
      expect(tags, <String>{'v1.0.0', 'v2.0.0'});
    });

    test('loadTagsFile throws for missing file', () {
      expect(() => SelectionService.loadTagsFile('/tmp/gfrm-missing-tags-file.txt'), throwsStateError);
    });
    ...
    test('applyTagsFilter returns only allowed tags', () {
      ...
      final List<String> filtered = SelectionService.applyTagsFilter(<String>['v1.0.0', 'v1.5.0', 'v2.0.0'], tagsPath);
    ...
    test('reserveOutputName sanitizes and prevents duplicates', () {
      ...
      final String first = SelectionService.reserveOutputName(used, 'path/to/my file.tar.gz');
      final String second = SelectionService.reserveOutputName(used, 'path/to/my file.tar.gz');
